...
<%
	...
	//----获取图片信息
	TPicture picture = (TPicture)request.getAttribute("picture");
	if (picture != null) {
		session.setAttribute("pictureId", picture.getPictureId());//保存段子id-给登录操作
	}
	else {
		response.sendRedirect("picture/"+session.getAttribute("pictureId")+".html");
	}
...
<body class="page-article">
	...
	<div class="main">
		<div class="left-wrapper">
			<h2 class="joke-title">图片内容</h2>
			<div class="content-bd"><%= picture.getPictureDesc()%></div>
			<div style="width:650px;text-align:center;">
				<img src="<%= picture.getPictureImg()%>" alt="picture" style="width:650px;margin:0 auto;"/>
			</div>
			
			<div class="interact">
				<a class="article-like"><%= picture.getPictureGood()%>赞</a>
...
<script type="text/javascript">
 	$(document).ready(function () {
		...
		$.ajax({
			type : "post",
			url : "picComment/getPicCommentsAndUsersByPictureId_ajax",
			data : {
		...
		$(".article-like").click(function () {
			...
			if (flag == '0') {
				...
				$(".article-like").html("<%= picture.getPictureGood()+1%>赞");
				$(".article-like").addClass("hasliked");
				$("#info").html("点赞成功！");
				$(".information-dialog").css("display", "block");
				$(".information-dialog").delay(2000).hide(0);
		...
		if (<%= flag%> == 1) {
			$(".article-report").click(function () {
				if ($(this).hasClass("article-hasliked")) {
					...
					$.ajax({
						type : "post",
						url : "picture/insertCollectPicture_ajax",
						data : {
							"userId" : "<%= user.getUserId()%>",
							"pictureId" : "<%= picture.getPictureId()%>"
						},
		...
		if (<%= flag%> == 1) {
			$(".add-comment-btn").click(function() {
				...
				if (textLen == 0 || textLen > 420) {
					...
					comment.pictureId = '<%= picture.getPictureId()%>';
					
					$.ajax({
						type : "post",
						url : "picComment/addPicComment_ajax",
						data : {
		...
		$(".comments-list").on("click", ".comment-delete", function () {
			 $.ajax({
				type : "post",
				url : "picComment/removePicComment_ajax",
				data : {
		...
		$(".comments-list").on("click", ".comment-like", function () {
			...
			if (flag == '0') {
				...
				$.ajax({//异步保存点赞评论数据
					type : "post",
					url : "picComment/addGoodPicComment_ajax",
					data : {
	...
	function fnCallbackComment(data) {//评论回调函数
		...
		if (page == null) {
			...
			if (<%=flag%> == 1) {
				...
				$.ajax({//异步获取点赞信息
					type : "post",
					url : "picComment/getGoodPicCommentInfo_ajax",
					dataType:"json",
					data : {
